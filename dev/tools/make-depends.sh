...
coqdep -R theories Stdlib theories | sed -n -e 's,/,.,g;s/[.]vo.*: [^ ]*v//p' | \
...
| sed -e 's/"Numbers[.]Arith[.][^"]*"/"coq-stdlib-arith-base"/g' \
...
| sed -e 's/"Numbers[.]Lists[.][^"]*"/"coq-stdlib-list"/g' \
...
| sed -e 's/"Numbers[.]positive[.][^"]*"/"coq-stdlib-orders-positive"/g' \
...
| sed -e 's/"Program[.][^"]*"/"coq-stdlib-program"/g' \
...
| sed -e 's/"Relations[.][^"]*"/"coq-stdlib-relations"/g' \
...
awk -- '{ if ($1 != substr($3, 1, length($3)-1)) { print $0 } }' depends_core;
echo "}") | tred > depends
